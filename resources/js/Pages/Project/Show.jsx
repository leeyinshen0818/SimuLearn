    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            {selectedTask && (
                <div className="fixed inset-0 z-50 overflow-y-auto" style={{ zIndex: 9999 }} aria-labelledby="modal-title" role="dialog" aria-modal="true">
                    <div className="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center sm:p-0">
                        <div className="fixed inset-0 bg-transparent transition-opacity" aria-hidden="true" onClick={closeModal}></div>

                        <div className="relative inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-gray-100">
                            ...
                            <div className="px-6 py-6">
                                <div className="prose prose-sm max-w-none text-gray-600 mb-6">
                                    <p className="text-base leading-relaxed">{selectedTask.description}</p>
                                </div>

                                <div className="grid grid-cols-1 gap-6">
                                    {selectedTask.scenario && (
                                        <div className="bg-gray-50 p-4 rounded border border-gray-100">
                                            <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Scenario</h4>
                                            <p className="text-sm text-gray-700 italic">{selectedTask.scenario}</p>
                                        </div>
                                    )}
                                    ...
                                    {selectedTask.skills && selectedTask.skills.length > 0 && (
                                        <div>
                                            <h4 className="text-sm font-semibold text-gray-900 mb-2">Recommended Skills</h4>
                                            <div className="flex flex-wrap gap-2">
                                                {selectedTask.skills.map(skill => (
                                                    <span key={skill.id} className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium border ${userSkills.includes(skill.id) ? 'bg-green-50 text-green-700 border-green-200' : 'bg-yellow-50 text-yellow-700 border-yellow-200'}`}>
                                                        {skill.name}
                                                    </span>
                                                ))}
                                            </div>
                                            ...
                                            <div className="flex">
                                                <div className="ml-3">
                                                    <p className="text-sm text-red-700">
                                                        This task is currently locked. You need to complete the prerequisite tasks to unlock this task.
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
